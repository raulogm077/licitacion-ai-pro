...
**Dos trampas al validar bumps, aprendidas el 2026-07-27.** Primera: el paso Deno de `verify:release` aplica una política de antigüedad mínima de 24 h sobre `package.json`, así que una versión publicada hace horas falla ahí aunque lint, tests, build y E2E hayan pasado (caso real: `jsdom@30.0.0`, publicado esa misma mañana). No se salta con `--minimum-dependency-age 0`; se espera y se revalida. Segunda: `pnpm verify:release > log 2>&1; echo $?` devuelve el estado del `echo`, no el de `pnpm` — comprobar la última línea del log (`verify:release completado` frente a `ELIFECYCLE`) antes de dar un lote por bueno.
