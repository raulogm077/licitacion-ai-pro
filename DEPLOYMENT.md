...
- `20260807120000_empresa_perfil_licitador.sql` — cuatro tablas del perfil del licitador (ADR-002 Paso 1): `empresa_perfil`, `empresa_ejercicio`, `empresa_proyecto`, `empresa_acreditacion`. **Owner-scoped de lectura y escritura**, al contrario que las tablas de análisis: este dato lo introduce el usuario, no el backend. Las hijas cuelgan de `perfil_id` y **ninguna declara `user_id`** — es la decisión 7.1 de la ADR y lo único que separa «añadir una columna» de «reescribir el modelo» el día que el perfil pase a ser de organización; `verify:integrity` falla si alguien lo rompe. Ninguna columna de datos lleva `DEFAULT` numérico ni `NOT NULL` con relleno, para que «no lo sé» siga siendo distinguible de «cero» hasta el motor de Go/No-Go (decisión 7.4). Índice GIN sobre `cpv` porque el filtro por prefijo recorre todo el histórico del licitador en cada evaluación.
...
**`chat-with-analysis-agent` y el perfil del licitador.** Desde el 2026-08-07 la función lee las tablas `empresa_*` del usuario de la sesión para calcular el Go/No-Go. Lo que devuelve al modelo es el veredicto, nunca las cifras: si alguien añade un campo al payload de `get_go_no_go`, tiene que comprobar que no venga del perfil. El test `tools_test.ts` lo verifica por valor. Detalle en `TECHNICAL_DOCS.md` §7.17.
