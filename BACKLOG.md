...
- [ ] Limpieza de código legacy: Revisar `supabase/functions/` y eliminar código viejo de colas (ej. `openai-runner`). Limpiar `startJob` y `pollJob` en `src/services/job.service.ts` a favor de `analyzeWithAgents`.
- [ ] Refactor de Tests: Mockear el módulo de config en tests de setup para silenciar las advertencias de Zod/EnvSchema en Vitest.
