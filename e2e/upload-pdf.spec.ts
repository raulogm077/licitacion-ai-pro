...
function buildMockSseStream(): ReadableStream {
    return new ReadableStream({
        start(controller) {
            ...
            setTimeout(() => {
                send({ type: 'phase_progress', message: 'Leyendo documento PDF...', timestamp: Date.now() });
            }, 80);
            ...
            setTimeout(() => {
                send({ type: 'phase_progress', message: 'Extrayendo datos generales...', timestamp: Date.now() });
            }, 240);
