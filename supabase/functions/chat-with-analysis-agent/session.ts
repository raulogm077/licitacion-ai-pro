import type { AgentInputItem } from '../_shared/agents/sdk.ts';
...
export async function getConversationHistory(sessionId: string, deps: SessionStoreDeps): Promise<AgentInputItem[]> {
    ...
    return structuredClone((data ?? []).map((row) => row.content as AgentInputItem));
}
...
export async function replaceConversationHistory(
    ...
    const { error: insertError } = await deps.supabase.from('analysis_chat_messages').insert(rows as never);
