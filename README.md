...
- `pnpm verify:integrity` comprueba, además del drift de migraciones y la cobertura documental, dos invariantes estructurales del perfil del licitador: que las cuatro tablas `empresa_*` tengan RLS **con** política y que ninguna tabla hija declare `user_id` en vez de `perfil_id` (ADR-002 §7.1). Son fallos silenciosos —no producen síntoma en runtime— y por eso se comprueban sobre el SQL.
...
- Remediación 2026-08-07b: dos avisos por debajo del umbral del CI, cerrados igualmente porque tenían parche alcanzable. `GHSA-4x5r-pxfx-6jf8` (`@babel/core`, LOW) se cierra dentro de la misma línea con el override `>=7.29.7 <8`. `GHSA-w5hq-g745-h8pq` (`uuid`, MODERATE) **no tiene parche en la línea 8.x** que arrastra `exceljs@4.4.0`, así que el override `>=11.1.1` cruza la frontera de major. Cuando un override cruza un major, el audit en verde no basta: hay que verificar funcionalmente al consumidor. Aquí se comprobó que el único módulo de `exceljs` que usa `uuid` sigue cargando y que un `.xlsx` con formato condicional hace roundtrip de escritura y lectura.
