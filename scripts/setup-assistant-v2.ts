...
if (!apiKey) {
    console.error('Error: OPENAI_API_KEY is not set in .env or .env.local');
    process.exit(1);
}
...
// --- CONFIGURATION ---
const ASSISTANT_NAME = "Analista de Pliegos - Antigravity V2";
const MODEL = "gpt-4o-2024-08-06";
const GUIA_FILENAME = "guia_lectura.pdf";
const VECTOR_STORE_NAME = "Base de Conocimiento Antigravity";

// 1.1 SYSTEM INSTRUCTIONS
const INSTRUCTIONS = `Eres “Analista de Pliegos (v2)”. Lees un pliego de licitación (PDF indexado) y extraes información siguiendo la guía interna de lectura (también indexada). NO das asesoramiento legal ni interpretaciones jurídicas: solo extraes hechos del pliego y los estructuras en el JSON canónico. La guía sirve para saber “cómo leer” y qué buscar; el pliego sirve para “qué es verdad”.
                ...
                "result": {
                    ...
                    "properties": {
                        "datosGenerales": {
                            ...
                            "required": ["titulo", "presupuesto", "moneda", "plazoEjecucionMeses", "cpv", "organoContratacion"],
                    ...
                    "required": ["datosGenerales", "criteriosAdjudicacion", "requisitosTecnicos", "requisitosSolvencia", "restriccionesYRiesgos", "modeloServicio"],
                    "additionalProperties": false
                },
                "workflow": {
                    "type": "object",
                        ...
                        "quality": {
                            ...
                            "properties": {
                                ...
                                "bySection": {
                                    ...
                                    "required": ["datosGenerales", "criteriosAdjudicacion", "requisitosSolvencia", "requisitosTecnicos", "restriccionesYRiesgos", "modeloServicio"],
                                    "additionalProperties": false
                                },
                            ...
                            "required": ["overall", "bySection", "missingCriticalFields", "ambiguous_fields", "warnings"],
                            "additionalProperties": false
                        },
                        "evidences": {
                            "type": "array",
...
async function main() {
    console.log("🚀 Starting Analista de Pliegos V2 Setup...");

    try {
        // 1. Check/Upload Guide
        const guidePath = path.resolve(process.cwd(), GUIA_FILENAME);
        if (!fs.existsSync(guidePath)) {
            throw new Error(`Guía file not found at: ${guidePath}`);
        }
        console.log(`✅ Found guide: ${GUIA_FILENAME}`);

        const fileStream = fs.createReadStream(guidePath);
        const uploadedFile = await openai.files.create({
            file: fileStream,
            purpose: "assistants",
        });
        console.log(`✅ Uploaded guide file. ID: ${uploadedFile.id}`);

        // 2. Create Vector Store
        const vectorStore = await openai.beta.vectorStores.create({
            name: VECTOR_STORE_NAME,
        });
        console.log(`✅ Created Vector Store: ${vectorStore.name} (${vectorStore.id})`);

        await openai.beta.vectorStores.files.create(vectorStore.id, {
            file_id: uploadedFile.id
        });
        console.log(`✅ Attached guide to Vector Store.`);

        // 3. Create/Update Assistant
        // Note: For simplicity in this script, we create a NEW one. 
        // In production, you might want to find and update by name or ID.
        console.log("Creating new Assistant...");
        const assistant = await openai.beta.assistants.create({
            name: ASSISTANT_NAME,
            description: "Asistente experto en análisis de licitaciones públicas",
            model: MODEL,
        ...
        console.log("===================================");
        console.log(`\n👉 Add this ID to your .env file as VITE_OPENAI_ASSISTANT_ID`);
