...
- Se ha detectado un **timeout en E2E (`locator.setInputFiles: Timeout 15000ms exceeded`)** al intentar localizar `input[type="file"]`. Esto puede deberse a que el input está marcado como `hidden` (`className="hidden"`) y Playwright podría no interactuar correctamente si no está forzado, o porque la simulación de sesión (`mock-user-id`) no resuelve la condición `isAuthenticated` en el UI a tiempo.
