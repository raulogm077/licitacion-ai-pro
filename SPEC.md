...
### QA: E2E Playwright Tests Bugfix (Iteración Actual)
- **Problema:** En el archivo `e2e/upload-pdf.spec.ts` los tests experimentaban timeouts excesivos (TimeoutError: locator.waitFor: Timeout 15000ms exceeded). Adicionalmente, existían referencias a `__dirname` incompatibles con el entorno ESM.
- **Acción Realizada:** Se actualizó `e2e/upload-pdf.spec.ts` para usar `import.meta.dirname` en lugar de `__dirname`. Además, se implementó una verificación y ejecución explícita de autenticación UI (esperar a que el botón "Iniciar Sesión" aparezca, y si lo hace, rellenar el formulario e iniciar sesión) antes de buscar el elemento de entrada de archivos. Esto asegura que la sesión se establezca correctamente y que los elementos dependientes de la autenticación estén presentes antes de interactuar con ellos, resolviendo el problema de timeouts.
