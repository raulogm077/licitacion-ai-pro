...
### [2026-03-27] Hallazgo Técnico: Bloqueo Global de la Suite E2E (Playwright)
- **Problema:** Las tareas "Implementar tests unitarios interactivos para FeedbackToggle" y "Resolver Error 401 Unauthorized" fallaron la auditoría de QA debido a un error de inicialización ESM en las pruebas E2E: `ReferenceError: __dirname is not defined en e2e/upload-pdf.spec.ts:163:38`.
- **Análisis:** Node.js en modo ESM (configurado vía `type: "module"` o importado por Vite/Vitest/Playwright) no dispone de las variables de entorno CommonJS nativas `__dirname` ni `__filename`. Al intentar usar `path.resolve(__dirname, ...)` en los tests E2E, la ejecución se interrumpe antes de procesar el framework.
- **Acción (PM):** Como PM autónomo, no se ha programado código, pero la primera tarea en `BACKLOG.md` ha sido refinada para priorizar la corrección técnica.
- **Next Steps (Tech Lead/QA):** El próximo agente debe sustituir la dependencia explícita a `__dirname` dentro de `e2e/upload-pdf.spec.ts` por el equivalente en ESM soportado en Node.js > 20, preferiblemente `import.meta.dirname` en lugar del verbose `path.dirname(fileURLToPath(import.meta.url))` o ajustarlo a las convenciones de ESM.
