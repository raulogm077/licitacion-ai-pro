...
## Resolver Bloqueo Global de Vitest (Tech Lead)
- **Implementación real:** Se ejecutó `pnpm install` para restaurar `node_modules` que estaban ausentes y causaban el error `vitest: not found`. Tras esto, se ejecutó con éxito `pnpm test`, pasando las 57 suites y 385 tests exitosamente sin errores de ESM ni bloqueos globales de Vitest. No fue necesario modificar código fuente de la aplicación, ya que el problema era una falta de hidratación de dependencias en el entorno. Adicionalmente, se ejecutó todo el pipeline QA local (`pnpm run typecheck`, `pnpm run build && xvfb-run pnpm run test:e2e`) exitosamente confirmando la estabilidad del proyecto.
- **Decisiones tomadas:** Se validó el comando global `pnpm test` en vez de apuntar a `vitest` directo, confirmando que la configuración actual en `package.json` y `vitest.config.ts` es correcta si `node_modules` está presente.
- **Limitaciones o riesgos residuales:** Ninguno conocido. El pipeline CI deberá tener un paso de install o caché robusto (como ya lo tiene configurado).
